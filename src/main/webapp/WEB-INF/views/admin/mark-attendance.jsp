        ...
        <form action="${pageContext.request.contextPath}/admin/attendance"
            ...
            <div class="mb-3">
                <button type="button" class="btn btn-sm btn-success"
                        onclick="markAll('P')">✅ Mark All Present</button>
                <button type="button" class="btn btn-sm btn-danger ms-2"
                        onclick="markAll('A')">❌ Mark All Absent</button>
            </div>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <input type="radio"
                                       ...
                                       ${existingMap[s.studentId] == 'P' ? 'checked' : ''}
                                       ${empty existingMap[s.studentId] ? 'checked' : ''}>
                            </td>
                            <td class="text-center">
                                <input type="radio"
                                       name="status_${s.studentId}"
                                       value="A"
                                       class="form-check-input status-radio"
                                       ${existingMap[s.studentId] == 'A' ? 'checked' : ''}>
                            </td>
                            <td class="text-center">
                                <input type="radio"
                                       name="status_${s.studentId}"
                                       value="L"
                                       class="form-check-input status-radio"
                                       ${existingMap[s.studentId] == 'L' ? 'checked' : ''}>
                            </td>
                        </tr>
                    </c:forEach>
                </tbody>
            ...
            <button type="submit" class="btn text-white px-5"
                    style="background:#1a3a5c;">💾 Save Attendance</button>
        </form>
    </c:if>
